...
assert.doesNotMatch(job, /if \(seed\.concrete_subject\) priorSubjects\.push/);
...
assert.match(livedSrc, /livedGroundingLabel/);
assert.doesNotMatch(livedSrc, /abstractLivedSubject/);
...
const lived = {
  ...
  concrete_subject: "lived grounding · FSD · yesterday",
  cluster: "FSD",
};
const pub = {
  seed_id: "pub-1",
...
for (const seed of packets) {
  assert.match(String(seed.concrete_subject), /^lived grounding · /);
  assert.equal(String((seed as any).source_role), "GROUNDING_EVIDENCE");
  assert.doesNotMatch(String(seed.point_or_tension), /보행자|야간이라/);
}

assert.match(livedGroundingLabel("FSD", "2026-08-01T20:00:00.000Z"), /^lived grounding · FSD · /);
