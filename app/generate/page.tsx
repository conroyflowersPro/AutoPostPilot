...
export default function GeneratePage() {
  ...
  async function runGeneration(
    existingJob?: GenerationJobState | null
  ) {
    setLoading(true);
    setError(null);
    setResult(null);
    ...
    try {
      ...
      let mergedKeywords =
        existingJob?.mergedKeywords || keywords.trim();
      ...
      if (!existingJob) {
        ...
        try {
          ...
          try {
            // Dedupe only — Vision v3: do NOT treat drafts as learning signal.
            // Prefer scheduled/reviewed/published; fill with drafts only if needed.
          ...
          const planRes = await fetch("/api/grok/plan", {
            ...
            body: JSON.stringify({
              ...
              interests:
                audienceInterests.length > 0 ? audienceInterests : undefined,
          ...
          <div className="rounded-xl border border-amber-700/50 bg-amber-950/30 p-4 text-sm">
            ...
            <div className="mt-3 flex gap-2">
              <button
                ...
                className="rounded-lg bg-amber-700 px-3 py-1.5 text-xs hover:bg-amber-600"
              >
                이어서 생성
              </button>
              <button
