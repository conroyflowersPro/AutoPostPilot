  ...
  async function runGeneration(existingJob?: GenerationJobState | null) {
    ...
    try {
      ...
      for (let di = startFrom; di < generationDays; di++) {
        ...
        setPhase(
          `Day ${di + 1}/${generationDays} 일괄 생성 중... (슬롯 ${day.posts.length}개 · 누적 ${allPosts.length})`
        );
        saveJob({
          jobId,
          startDate,
    ...
    <div className="min-h-screen bg-zinc-950 text-zinc-100">
      ...
      <main className="mx-auto max-w-3xl space-y-6 px-4 py-6">
        <p className="text-sm text-zinc-400">
          Grok이 7일 주간 운영 캘린더를 짠 뒤 날짜별 일괄 생성합니다. Fedica 키워드는 글에 넣지 않고 관심사 신호로만 씁니다.
        </p>
        ...
        <form onSubmit={handleGenerate} className="space-y-4">
          ...
          <div>
            <label className="mb-1.5 block text-xs text-zinc-400">
              Fedica 키워드 / 테마 (선택 · 글에 넣지 않음, 관심사 신호만)
            </label>
            <input type="text" value={keywords} onChange={(e) => setKeywords(e.target.value)} placeholder="예: Terafab, Optimus, Grimes County (복붙 금지 · 테마로 변환됨)" className="w-full rounded-xl border border-zinc-700 bg-zinc-900 px-4 py-3 text-sm outline-none focus:border-emerald-500" />
          </div>
          ...
          {error && (
            <div className="space-y-2 rounded-lg bg-red-900/30 px-3 py-3">
              ...
              <button type="button" onClick={() => router.push("/")} className="rounded-lg bg-zinc-700 px-3 py-1.5 text-xs hover:bg-zinc-600">현황으로 돌아가기 →</button>
            </div>
          )}
          ...
          <button type="submit" disabled={loading} className="w-full rounded-xl bg-indigo-600 py-3 text-sm font-medium hover:bg-indigo-500 disabled:opacity-50">
            {loading ? phase || "처리 중..." : "7일 한국어 주간 운영 자동 생성"}
          </button>
        </form>
