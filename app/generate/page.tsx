...
function GeneratePageInner() {
  ...
  async function runGeneration() {
    ...
    try {
      ...
      try {
        const {
          data: { session: planSession },
        } = await supabase.auth.getSession();
        if (!planSession?.access_token) throw new Error("로그인이 필요합니다");
        const supabaseUrl = process.env.NEXT_PUBLIC_SUPABASE_URL;
        if (!supabaseUrl) throw new Error("NEXT_PUBLIC_SUPABASE_URL 없음");
    ...
    <div className="mx-auto max-w-3xl space-y-6 px-4 py-6">
      <div>
        ...
        <p className="mt-1 text-sm text-zinc-400">
          키워드·스샷은 관심사 신호로만 사용합니다. Planner는 Supabase Edge(엔진 연결 유지)에서
          실행됩니다. 실패 시 자동 대체 초안을 만들지 않습니다.
        </p>
          ...
          <div className="rounded-xl border border-emerald-800/50 bg-emerald-950/30 p-4 text-sm space-y-2">
            ...
            {result.dna_sources && (
              <p className="text-[10px] text-zinc-500">
                ...
                {result.dna_sources.runtime ? ` · runtime=${result.dna_sources.runtime}` : ""}
              </p>
            )}
            {result.plan && (
