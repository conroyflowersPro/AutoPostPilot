...
import { getDemoActivities } from "@/lib/calendar/demo-data";
...
export default function CalendarControlCenter() {
  const [viewMonth, setViewMonth] = useState(() => startOfMonth(new Date()));
  const [selectedDate, setSelectedDate] = useState<Date | null>(new Date());
  const [copiedId, setCopiedId] = useState<string | null>(null);

  const activities = useMemo(() => getDemoActivities(viewMonth), [viewMonth]);
    ...
    <div className="space-y-4">
      <div className="rounded-xl border border-zinc-800 bg-zinc-900/60 p-4">
        ...
        <div className="grid grid-cols-2 gap-2 sm:grid-cols-3 md:grid-cols-6 text-sm">
          ...
          <div className="rounded-lg bg-zinc-800/80 px-3 py-2">
            ...
            <div className={`font-semibold ${summary.manualActions > 0 ? "text-amber-400" : ""}`}>
          ...
          <div className="rounded-lg bg-zinc-800/80 px-3 py-2">
            ...
            <div className={`font-semibold ${summary.duplicateWarnings > 0 ? "text-rose-400" : ""}`}>
      ...
      <div className="overflow-hidden rounded-xl border border-zinc-800">
        ...
        <div className="grid grid-cols-7 bg-zinc-950">
          {days.map((day) => {
            ...
            const hasManual = acts.some(
              (a) => a.manual_action_required || a.status === "MANUAL_ACTION_REQUIRED"
            );
            const hasDup = acts.some((a) => !!a.duplicate_warning);
            const high = acts.length >= 3;
            ...
            return (
              <button
                ...
                className={`min-h-[72px] border-b border-r border-zinc-800/80 p-1 text-left transition-colors
                  ${!inMonth ? "bg-zinc-950/40 text-zinc-600" : "hover:bg-zinc-900"}
                  ${selected ? "bg-indigo-950/50 ring-1 ring-inset ring-indigo-500/50" : ""}
                  ${today && !selected ? "bg-zinc-900/80" : ""}
                `}
              >
                <div className="mb-0.5 flex items-center justify-between px-0.5">
                  <span
                    className={`text-xs font-medium ${
                      today ? "rounded-full bg-indigo-600 px-1.5 text-white" : ""
                    }`}
                  >
                    {format(day, "d")}
                  </span>
                  <span className="flex gap-0.5">
                    ...
                    {high && <span className="text-[9px] text-zinc-500">●</span>}
                  </span>
                </div>
                <div className="flex flex-col gap-0.5">
                  {acts.slice(0, 3).map((a) => (
                    <div
                      key={a.activity_id}
                      className={`truncate rounded px-1 py-0.5 text-[9px] leading-tight text-white ${ORIGIN_COLOR[a.origin]}`}
                      title={`${ORIGIN_LABEL[a.origin]} · ${a.status}`}
                    >
                      {ORIGIN_LABEL[a.origin]}
                    </div>
                  ))}
                  {acts.length > 3 && (
                    <div className="text-[9px] text-zinc-500">+{acts.length - 3}</div>
                  )}
                </div>
              </button>
            );
      ...
      <div className="rounded-xl border border-zinc-800 bg-zinc-900/40 p-4">
        ...
        {selectedActs.length === 0 && (
          <p className="text-sm text-zinc-500">이 날짜에 등록된 활동이 없습니다.</p>
        )}
          ...
          {selectedActs.map((a) => (
            ...
            >
              <div className="mb-2 flex flex-wrap items-center gap-2">
                <span className={`rounded px-1.5 py-0.5 text-[10px] text-white ${ORIGIN_COLOR[a.origin]}`}>
                  {ORIGIN_LABEL[a.origin]}
                </span>
              ...
              {a.performance_summary && (
                <div className="mb-2 text-xs text-emerald-400">{a.performance_summary}</div>
              )}
              ...
              {(a.status === "MANUAL_ACTION_REQUIRED" || a.manual_action_required) && (
                <div className="mt-2 flex flex-wrap gap-2 border-t border-zinc-800 pt-2">
                  {a.generated_text && (
                    <button
                      type="button"
                      onClick={() => copyText(a.activity_id, a.generated_text || "")}
                      className="inline-flex items-center gap-1 rounded-md bg-zinc-800 px-2.5 py-1.5 text-xs hover:bg-zinc-700"
                    >
                      {copiedId === a.activity_id ? (
                  ...
                  <button
                    type="button"
                    className="rounded-md bg-emerald-700/80 px-2.5 py-1.5 text-xs hover:bg-emerald-600"
                    title="데모: 실제 완료 처리는 이후 연동"
                  >
                    Mark Completed
                  </button>
                  <button
                    type="button"
                    className="rounded-md bg-zinc-800 px-2.5 py-1.5 text-xs text-zinc-400 hover:bg-zinc-700"
                    title="데모"
                  >
                    Dismiss
                  </button>
