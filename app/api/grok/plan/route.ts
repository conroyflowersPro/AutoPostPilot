...
- strategicAngle, hookStyle, writingApproach
- experienceUsage / opinionStrength / observationLevel / technicalDepth / emotionalLevel / predictionLevel
- questionUsage, ctaUsage (default false unless natural)
- targetGrowthObjective: followers | profile_visits | bookmarks | replies | authority | expansion | balanced
- mediaUsefulness, hypothesisNote
Do NOT force every post to have strong hook + question + CTA + personal story.
Pick the strategy that best fits this topic and growth objective this week.
X Strategy is NOT a sentence formula. It is the growth role of the post.
...
- Performance DNA = validated published patterns only (including strategy→outcome when available).
- Never write: "gaming performed well → Creator is a gaming creator."

PIPELINE
1) Candidate topics (~80–100), creator-native, broad pool (not only FSD/CT).
2) Score with Audience DNA + Creator Intent + expansion value + diversity.
3) Adjust with Performance DNA (validated only).
4) Build 7-day ORIGINAL slots (~5–6/day preferred, 5–8 max).
5) Assign Post Strategy per slot.
6) Portfolio self-check: identityStatement, expansionMoves, riskOfNarrowing, creatorIntentReflection.
7) If portfolio is too narrow, replace slots before final JSON.
...
Output JSON only:
{
  ...
  "audienceRead": {
    ...
    "coverageNote": "한국어 한 줄",
    "performanceLean": "한국어 한 줄",
    "topicDistribution": {"theme": 3},
    "creatorIntent": "이번 주 Creator Intent를 어떻게 반영했는지 한국어 한 줄",
    "portfolio": {
      "identityStatement": "이번 주 전체를 따라읽으면 이 Creator는 …",
      "expansionMoves": ["..."],
      "diversityNotes": "...",
      "riskOfNarrowing": "low|medium|high — 이유",
      ...
      "posts": [
        {
          ...
          "postStrategy": {
            "strategicAngle": "…",
            "hookStyle": "…",
            "writingApproach": "observation|opinion|personal_experience|comparison|info_interpretation|technical|prediction|question|mixed",
            "experienceUsage": "none|low|medium|high",
            "opinionStrength": "none|low|medium|high",
            "observationLevel": "none|low|medium|high",
            "technicalDepth": "none|low|medium|high",
...
Performance DNA (validated only — may include strategy→outcome if present): ${typeof performanceHint === "string" ? performanceHint.slice(0, 1200) : JSON.stringify(performanceHint).slice(0, 1200)}
...
If portfolio risks narrowing into FSD/Cybertruck only, expand before final output.`;
    ...
    if (days.length === 0) {
      // minimal structure so downstream still works
      days = Array.from({ length: daysCount }, (_, i) => ({ dayOffset: i, posts: [] }));
    }

    // Normalize each day/slot
    ...
    // Server-side diversity guard (v5.4.1)
    ...
    const audienceRead = {
      ...
      portfolio: {
        ...
        identityStatement:
          parsed?.audienceRead?.portfolio?.identityStatement ||
          "주간 포트폴리오 자동 점검",
        diversityNotes: diversity.note || portfolioStats.noteKo,
        riskOfNarrowing:
          parsed?.audienceRead?.portfolio?.riskOfNarrowing ||
          portfolioStats.narrowingRisk,
        creatorIntentReflection:
          parsed?.audienceRead?.portfolio?.creatorIntentReflection ||
          (topic
            ? intentOk
              ? "Creator Intent 신호가 주제/앵글에 반영됨"
              : "Creator Intent 입력됨 — 반영 약함, 다음 사이클 강화"
      ...
      creatorIntent:
        parsed?.audienceRead?.creatorIntent ||
    ...
    return NextResponse.json({
      ...
      audienceRead: {
        portfolio: {
          ...
          expansionMoves: diversity.changed
            ? ["fallback 다양성 가드레일 적용"]
            : [],
        },
        diversityGuardApplied: diversity.changed,
      },
