...
function materialize(prefix, destRel, expectedParts) {
  const chunks = [];
...
const m = materialize("everyday-language-reasoning.ts", "supabase/functions/weekly-plan/everyday-language-reasoning.ts", 6);
const t = materialize("order5a-everyday-language-test.mjs", "tools/order5a-everyday-language-test.mjs", 5);
if (m < 20000 || t < 15000) {
  console.error("size check failed", m, t);
  process.exit(1);
}
