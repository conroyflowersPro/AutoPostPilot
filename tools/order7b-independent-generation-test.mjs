...
function makeCtx(overrides = {}) {
  ...
  return {
    slot_id: slot, context_id: ctxId,
    seed_identity: { seed_id: overrides.seed_id || "s1", concrete_subject: subject, cluster: overrides.cluster || "CYBERTRUCK", editorial_mode: "INFORMATIVE" },
    interpreted_meaning: { status: "INTERPRETATION_OK", seed_subject: subject, what_is_actually_happening: subject, why_it_matters_now: overrides.why || "실사용", human_element: overrides.human || "체감" },
    why_it_matters: overrides.why || "실사용", human_element: overrides.human || "체감",
    factual_boundaries: [],
    experience_boundaries: { creator_experienced: !!overrides.experienced, must_not_claim_first_person: overrides.must_not_claim_first_person !== false && !overrides.experienced },
    ...
    core_thought: { status: "CORE_THOUGHT_READY", primary_claim: `tension_around:${subject.slice(0, 40)}`, creator_judgment: "judgment_axis:current_seed_relevance", tension: overrides.tension || subject.slice(0, 40), useful_implication: "reader_bridge:open_inference", reader_relevant_meaning: subject, confidence: 0.7, evidence_dependency: "none", experience_dependency: false, source_meaning_separated: true, from_current_seed: true, block_reasons: [], order7a_version: "deep_generation_context_v1_order7a" },
    ...
    humor_decision: { humor_compatible: !!overrides.humor, humor_grounded: false, humor_strength: overrides.humor ? "LIGHT" : "NONE", self_deprecation_allowed: false, laughter_marker_allowed: false, punchline_compatible: false, punchline_required: false, sample_punchline: null, stop_after_punchline_ok: false, explanation_after_punchline_allowed: true, no_humor_is_normal: true },
    compression_target: overrides.compression || "NATURAL",
    reader_inference_space: "medium",
    stop_condition: { mechanism_completed_ok: true, core_thought_delivered_ok: true, punchline_stop_ok: false, leave_inference_open: true, avoid_explanatory_tail: true, minimal_context_sufficient: true },
    ...
    invariants: { question_required: false, cta_required: false, no_generation_template: true, no_ai_report_voice: true, no_reasoning_trace_in_output: true, per_post_isolation: true, generator_consumes_decisions: true },
...
function mirrorGenerate(ctx) {
  if (!ctx || ctx.generation_status === "GENERATION_CONTEXT_BLOCKED") return { generation_status: "GENERATION_CONTEXT_NOT_WRITABLE", final_text: "", context_id: ctx?.context_id, slot_id: ctx?.slot_id };
  ...
  if (ctx.compression_target === "VERY_COMPRESSED") body = body.slice(0, 90);
  if (ctx.experience_boundaries?.must_not_claim_first_person) body = body.replace(/제가 직접 써보니|어제 해봤는데|운전하다가/g, "");
  body = body.replace(/여러분은 어떠신가요|결국 중요한 것은/g, "");
  return { generation_status: "GENERATED", final_text: body.trim(), context_id: ctx.context_id, slot_id: ctx.slot_id, seed_fidelity: true, experience_boundary_preserved: !/제가 직접 써보니/.test(body), plan_markers: { humor_mode: ctx.humor_decision.humor_strength, mechanism_flexible: true, rail_flexible: true } };
}
const a1 = mirrorGenerate(makeCtx({ slot_id: "D1P1", subject: "Cybertruck 충전 속도", tension: "기대보다 체감" }));
const a2 = mirrorGenerate(makeCtx({ slot_id: "D1P2", subject: "Cybertruck 적재공간", tension: "짐 넣을 때" }));
const a3 = mirrorGenerate(makeCtx({ slot_id: "D1P3", subject: "Cybertruck 와이퍼 소음", tension: "비 올 때" }));
...
const b1 = mirrorGenerate(makeCtx({ slot_id: "D2P1", subject: "FSD 고속도로", mechanism: "RECOGNITION" }));
const b2 = mirrorGenerate(makeCtx({ slot_id: "D2P2", subject: "LAFC 직관", mechanism: "RECOGNITION", cluster: "LAFC" }));
...
const batch = [mirrorGenerate(makeCtx({ slot_id: "D3P1", subject: "Robotaxi 주정차" })), mirrorGenerate(makeCtx({ slot_id: "D3P2", subject: "S Plaid 가속" })), mirrorGenerate(makeCtx({ slot_id: "D3P3", subject: "도지 밈" })), mirrorGenerate(makeCtx({ slot_id: "D3P4", subject: "BMO 직관" })), mirrorGenerate(makeCtx({ slot_id: "D3P5", subject: "M3 퍼포먼스 서스펜션" }))];
ok("TestC. five distinct batch texts", new Set(batch.map((x) => x.final_text)).size === 5);
ok("TestC. no shared framing phrase across all", !batch.every((x) => x.final_text.includes("결국")));
...
ok("TestD. prohibited_copy_sources respected in markers", /ORDER7B_NO_MANUAL_PROSE_INPUT|manual_creator_posts|historical_post_text|prohibited_copy_sources/.test(mod));
const e1 = mirrorGenerate(makeCtx({ subject: "FSD 업데이트 노트", experienced: false, must_not_claim_first_person: true }));
ok("TestE. no fabricated first-person", !/제가 직접 써보니/.test(e1.final_text));
