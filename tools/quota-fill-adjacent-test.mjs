...
ok("A8. targeted refill is requested by Planner field direction", /planner_exploration_direction/.test(job) && /TARGETED_EXPLORE/.test(readFileSync(path.join(ROOT, "supabase/functions/weekly-plan/seven-day-planner.ts"), "utf8")));
...
ok("A11. shortfall uses Planner-targeted exploration, never a frozen keyword list", /Planner 지정 분야 Seed 추가 탐색/.test(job) && !/localHumorKeywordSeeds/.test(job));
ok("A12. Seed Generator explores adjacent and new areas without a fixed sector prompt", /adjacent areas, and plausible new expansion areas/.test(cr) && !/adjacentRingPromptLines\(\)/.test(cr));
ok("A13. Writer receives Seed boundary instead of adjacent writing instructions", /ASSIGNED SEED/.test(wr) && !/ADJACENT RING/.test(wr));
ok("A14. generate page keeps review copy", /리뷰하세요/.test(gen));
ok("A15. Planner infers Editorial Mode without fixed EXPERIENCE supply ratio", !/expSupply/.test(readFileSync(path.join(ROOT, "supabase/functions/weekly-plan/seven-day-planner.ts"), "utf8")));
ok("A16. Planner selects from existing Pool before requesting expansion", /Use the existing Seed pool first/.test(readFileSync(path.join(ROOT, "supabase/functions/weekly-plan/seven-day-planner.ts"), "utf8")));
ok("A17. Writer prevents unsupported first-person experience", /do not claim first-person lived experience/.test(wr));
ok("A18. live Planner has no fixed mass-public daily cap", !/MASS_PER_DAY_MAX/.test(readFileSync(path.join(ROOT, "supabase/functions/weekly-plan/seven-day-planner.ts"), "utf8")));
ok("A19. Seed prompt does not judge ticker/news strategic value", !/generic Tesla ticker/.test(cr));
ok("A20. Writer has no mass-slot topic override", !/MASS PUBLIC SLOT/.test(wr));
...
ok("A24. helper still classifies personal interest without controlling Planner", (() => {
  ...
  return personal === 6 && mass === 0 && placeable === 6;
})());
ok("A34. empty Grok expand uses compactRetry next tick, never DNA keyword inject",
  /compactRetry: compact/.test(job) && /Seed 짧게 재추론/.test(job) && !/DNA 관심 키워드/.test(job) && !/localHumorKeywordSeeds/.test(job));
ok("A35. Planner requests a bounded field when Pool lacks a fit",
  /result\.value\.missing/.test(job) && /planner_exploration_direction/.test(job) && !/localHumorKeywordSeeds/.test(job));
ok("A36. Seed Generator has no humorRing strategic specialization",
