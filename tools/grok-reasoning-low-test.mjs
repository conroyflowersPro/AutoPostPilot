...
ok("R2. planner uses reasoning_effort low", /reasoning_effort:\s*"low"/.test(readFileSync(path.join(ROOT, "supabase/functions/weekly-plan/seven-day-planner.ts"), "utf8")));
ok("R3. writer is Grok 4.6 with reasoning_effort low", /api\.x\.ai\/v1\/responses/.test(wr) && /reasoning_effort:\s*"low"/.test(wr) && /callGrokWriter/.test(wr));
ok("R4. expand abort labeled xai_timeout", /xai_timeout/.test(cr));
ok("R5. leftover quota phase is gone from live inference", !/inferWeeklyQuota/.test(ix) && !/quota_grok_error/.test(ix));
