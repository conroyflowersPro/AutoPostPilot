...
export function engineRulesAsWill(): string {
  return [
    ...
    "3-day generate infers seeds from learned data. Never emit DIMENSION_REGISTRY labels as seed bodies. Never emit a prompt example, canned phrase, or few-shot subject as concrete_subject.",
    "Infer the 3-day quota from Creator DNA + cadence + Performance DNA + X anti-dump, then fill that quota. Prefer 4/day; 5 fills the 14:00–22:00 PT window. Not a frozen 5. Always fill the inferred quota. Thin Grok supply is a reason to infer again, not to ship a short week. Quota holes must be filled by inferred seeds, with observational humor if needed. Never fill with a frozen keyword list or a prompt example.",
    ...
    "Writer (Grok 4.6) closes ONE central thought for this Seed from the Seed + Creator DNA + fact/experience boundaries, then writes it. Style, 말투, humor, Mechanism, and Rail follow. They must not choose the thought. Do not copy winning sentences; transfer abstract delivery only.",
    "USER_DIRECT trains 말투. AP_PIPELINE trains performance only and must not rewrite Creator DNA. The planner decides 해요/음슴/other per slot from DNA + engine + this 3-day set so far. No frozen mix percentage. Editorial mode is not a 말투 or length table. Write until the closed thought is complete.",
    ...
    "Engagement-bait closers are forbidden. A genuine thinking question inside the closed thought is allowed. Do not install a question because X rewards participation.",
