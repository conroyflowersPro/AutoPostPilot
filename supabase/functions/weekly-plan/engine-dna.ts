...
export function engineRulesAsWill(): string {
  return [
    ...
    "USER_DIRECT trains 말투. AP_PIPELINE trains performance only. The planner decides 해요/음슴/other per slot from DNA + engine + this 3-day set so far. No frozen mix percentage. Editorial mode is not a 말투 or length table. Write until the selected reader-entry move is complete.",
