...
export function creatorDnaBlock(): string {
  return [
    ...
    "PUBLISHING DNA: two-speed mix. 해요/존칭 and 음슴 are both publishing surfaces. Infer per post from handmade mix + DNA + engine. Editorial mode is not a 말투 table. Across the 3-day set, endings must not collapse to one register.",
...
export function engineRulesAsWill(): string {
  return [
    ...
    "USER_DIRECT trains 말투. AP_PIPELINE trains performance only. Infer 해요/음슴/other per post from DNA + engine + handmade mix + the 3-day batch so far. Editorial mode is not a 말투 or length table. Write until the selected reader-entry move is complete.",
