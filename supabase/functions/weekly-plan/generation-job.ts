...
function beginRecoverIfQueueReady(row: any, required: number): boolean {
  ...
  row.step = "recover";
  row.label_ko = `거절 ${batch.length}칸 승Grok 재판단 ${row.saved_count}/${required}…`;
  return true;
}
...
async function stepStrategy(supabase: any, xaiKey: string, row: any) {
  ...
  if (!st.planner_volume) {
    ...
    if (!result.ok || !result.value) {
      ...
      row.status = "error";
      row.error = `7일 승Grok 칸 수 실패: ${result.error || "unusable"}`;
    ...
    row.label_ko = `7일 칸 수 잠금 ${locked}칸 · 승Grok 슬롯…`;
    row.summary = [
      row.summary,
      analyticsLine,
      `승Grok 칸 수 ${locked} · 하루 ${result.value.posts_per_day.join("/")}`,
  ...
  if (days.length) {
    ...
    if (!result.ok || !result.value) {
      ...
      if (st.planner_day_batch_attempts < 3) {
        row.label_ko = `승Grok ${days.map((d) => d + 1).join(",")}일차 슬롯 재추론 ${st.planner_day_batch_attempts}/3…`;
        row.summary = [row.summary, `Creator day slots: ${result.error || "unusable"}`].filter(Boolean).join("\n");
        return;
      }
      row.status = "error";
      row.error = `7일 승Grok 슬롯 실패 (${days.map((d) => d + 1).join(",")}일차): ${result.error || "unusable"}`;
    ...
    if (remain.length) {
      row.label_ko = `승Grok 슬롯 ${st.planner_slots_partial.length}칸 · ${remain.map((d) => d + 1).join(",")}일차…`;
      return;
    }
  }
  ...
  row.summary = [
    row.summary,
    `승Grok 잠금 ${row.required_slots}칸 · Planner가 시각 배정 · Seed Generator에 ${seedTarget}개 요청 (칸 + ${SEED_POOL_BUFFER})`,
  ...
  if (!st.recover_relabeled) {
    ...
    if (!result.ok || !result.value) {
      if (isTransientXaiError(result.error)) {
        ...
        holdForXai(row, "xAI 응답 대기 · 승Grok 거절 재판단 이어감…", `Creator relabel: ${result.error}`);
        return;
      }
      row.label_ko = `승Grok 거절 재판단 재추론 ${pending.attempts}/4…`;
