...
const APP_VERSION = "10.0.0-order1";
const WEEKLY_ENGINE_VERSION = "phased_v10_order1_seed_interpretation";
  ...
  try {
    ...
    if (phase === "select") {
      ...
      for (const plannedMode of queue) {
        ...
        outDays[bestDay].posts.push(compactSlot(picked, bestDay, outDays[bestDay].posts.length + 1, mode, interp));
      }
      ...
      while (flatCount < baseNeed && pool.length > 0) {
        ...
        for (const m of tryModes) {
          ...
          outDays[minD].posts.push(compactSlot(seed, minD, outDays[minD].posts.length + 1, m as EditorialMode, interpFill));
