/**
 * Weekly Planner Edge v9.1.1 — expand timeout fix (fast model)
 */
  ...
  try {
    ...
    if (phase === "expand") {
      ...
      if (!skipXaiExpand && xaiKey) {
        ...
        const sys =
          "Expand topic dimensions into concrete Korean post seed subjects for Tesla/FSD/Cybertruck/LAFC creator. " +
          "JSON only: {seeds:[{cluster,dimension,concrete_subject,point_or_tension,editorial_mode}]}. " +
          "Specific subjects only. No HUMOR. Prefer INFORMATIVE,COMPARE,OPINION,CASUAL_OBSERVATION. " +
          "2 seeds per dimension. Korean.";
        const collected: any[] = [];
        ...
        for (let bi = 0; bi < batches.length; bi++) {
          ...
          const userP =
            ...
            `Need ~${Math.ceil(required_slots / batches.length)} seeds this batch.`;
          try {
            const raw = await callXai(xaiKey, sys, userP, 1400, EXPAND_TIMEOUT_MS, MODEL_EXPAND);
            xai_raw_len += String(raw || "").length;
            const parsed = extractJson(raw);
            if (parsed) xai_parse_ok = true;
      ...
      return json({
        ...
        note: "v9.1.1: expand uses grok-4.20-non-reasoning + 28s batches (avoid Edge abort).",
