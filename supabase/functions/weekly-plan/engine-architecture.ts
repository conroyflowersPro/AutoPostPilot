...
export function engineCatalogBlock(): string {
  return [
    "SEED: Seed Generator collects last-7-day Korean public posts (replies >= 20; impressions >= 50k only if that pool is short) as one scene + one observation. It does not judge RETURN/BRIDGE/REACH or types. PLANNING: Weekly Agent승 judges RETURN/BRIDGE/REACH and types. REACH is 1 per day, max 2. Consecutive slots must not share a situation cluster. FSD/driving scenes are at most 2 per day. Weekly Agent승 infers each slot date/time among 11:00, 15:00, 19:00 PT (3/day, 4-hour gap). Do not stamp a 14:00–22:00 2-hour grid. Recent-flow lived material is Analytics originals plus sync-gap originals. USER_DIRECT and AP_PIPELINE stay separate. Post Agent승 receives the assigned Seed + slot intent.",
