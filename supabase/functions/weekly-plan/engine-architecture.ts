...
export function engineCatalogBlock(): string {
  return [
    ...
    "QUALITY: Semantic Judge sees only the completed post and decides final publishability. It does not select Seeds, redesign strategy, or rewrite. Reject returns the slot to Planner; Planner reselects from the existing pool first and requests targeted Seed exploration only when needed. Count Gate requires N Judge-PASS posts for N Planner slots.",
