...
export function engineCatalogBlock(): string {
  return [
    "SEED: Seed Generator collects last-14-day Korean public posts (replies >= 20; impressions >= 50k only if that pool is short) as one scene + one observation. It does not judge RETURN/BRIDGE/REACH or types. PLANNING: Creator DNA judges RETURN/BRIDGE/REACH and types. REACH is 1 per day, max 2. Consecutive slots must not share a situation cluster. FSD/driving scenes are at most 2 per day. Planner places 14:00–22:00 PT and allocates Seeds. Recent-flow lived material is Analytics originals plus sync-gap originals. Writer receives the assigned Seed + Planner Intent.",
