...
export async function inferWeeklyQuota(args: {
  ...
  const system = [
    ...
    "Reference the X algorithm for STRATEGY only: anti-dump (stacked originals become noise; same-author decay), 48-hour For You freshness, mix, whether higher volume linked to growth.",
    "X ranking weights multiply predicted viewer-action probabilities, not raw engagement counts. They do not write posts and do not pick the last sentence.",
